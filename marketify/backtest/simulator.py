        ...
        if config.data.ticker in open_state:
            ...
            if exit_price is not None:
                ...
                trade_diagnostics.append({
                    "timestamp": str(ts),
                    "entry_timestamp": str(state.get("entry_timestamp", ts)),
                    "symbol": config.data.ticker,
                    "side": state["side"],
                    "qty": state["qty"],
                    "entry_price": entry_p,
                    ...
                    "entry_news_risk": state.get("entry_news_risk", 0.0),
                    "entry_news_sentiment_score": state.get("entry_news_sentiment_score", 0.0),
                    "entry_news_event_shock": state.get("entry_news_event_shock", 0.0),
                    "entry_macro_event_risk": state.get("entry_macro_event_risk", 0.0),
                    "entry_vix_proxy": state.get("entry_vix_proxy", 0.0),
                    "entry_vol_regime_ratio": state.get("entry_vol_regime_ratio", 0.0),
                    "entry_volatility_regime_code": state.get("entry_volatility_regime_code", 0.0),
                    "entry_signal_quality_20": state.get("entry_signal_quality_20", 0.0),
                    "entry_trend_alignment_score": state.get("entry_trend_alignment_score", 0.0),
                    "stop_loss": state["stop_loss"],
                    "take_profit": state["take_profit"],
        ...
        if idea is not None and config.data.ticker not in open_state:
            ...
            if decision.approved:
                ...
                open_state[config.data.ticker] = {
                    ...
                    "entry_news_risk": news_risk,
                    "entry_news_sentiment_score": sentiment_score,
                    "entry_news_event_shock": _scalar(row, "news_event_shock") if "news_event_shock" in feat.columns else 0.0,
                    "entry_macro_event_risk": _scalar(row, "macro_event_risk") if "macro_event_risk" in feat.columns else 0.0,
                    "entry_vix_proxy": _scalar(row, "vix_proxy") if "vix_proxy" in feat.columns else 0.0,
                    "entry_vol_regime_ratio": _scalar(row, "vol_regime_ratio") if "vol_regime_ratio" in feat.columns else 0.0,
                    "entry_volatility_regime_code": _scalar(row, "volatility_regime_code") if "volatility_regime_code" in feat.columns else 0.0,
                    "entry_signal_quality_20": _scalar(row, "signal_quality_20") if "signal_quality_20" in feat.columns else 0.0,
                    "entry_trend_alignment_score": _scalar(row, "trend_alignment_score") if "trend_alignment_score" in feat.columns else 0.0,
                    "peak_price": price,
                    "trough_price": price,
